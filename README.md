...
Codex API 排队、保活与手动请求服务，提供 Gin + React 管理控制台。多个 target 共用同一个任务队列、可配置并发上限和运行状态；OpenILink 与 Telegram Bot 都可作为消息入口，并可同时启用。
...
控制台展示 SSE、OpenILink、Telegram 状态、并发概览和每个 target 的排队/保活状态，并支持单个或批量“开始挤队列”“开始保活”“停止当前任务”。空闲 target 还可以执行一次性手动请求：输入 Prompt 后返回最终输出、Codex CLI 输出、退出码、耗时与错误。最大并发可在“任务与保活”中修改并立即生效；降低并发不会中断已经运行的请求。

任务规则：

- 排队失败后立即重新竞争全局并发槽，成功后停止该 target；旧重试区间仅为回滚兼容保留。
- 保活启动后立即请求，之后在随机区间内继续请求；失败只记录状态。
- 手动请求要求 target 当前空闲，并与排队、保活共用全局并发上限；关闭结果窗口或点击“终止请求”会取消当前 Codex 进程。退出码无法取得时返回 `-1`。
