...
function TargetCard({ target, selected, toggle, runAction, openAdhoc }: { target: Target; selected: boolean; toggle: () => void; runAction: (actionName: string, target: string) => void; openAdhoc: () => void }) {
  ...
  return <Card className="target-card"><CardHeader className="target-header"><div className="target-title"><Checkbox aria-label={`选择 ${target.name}`} checked={selected} onChange={toggle} /><div><div className="target-name-line"><CardTitle>{target.name}</CardTitle>{target.adhoc_running && <Badge>手动请求中</Badge>}</div><p className="muted">{target.model} · {target.api_host}</p></div></div><div className="target-actions"><Button size="sm" variant="default" disabled={target.adhoc_running} onClick={() => runAction("queue.start", target.name)}><Play size={14} />开始挤队列</Button><Button size="sm" variant="outline" disabled={target.adhoc_running} onClick={() => runAction("keepalive.start", target.name)}><Radio size={14} />开始保活</Button><Button size="sm" variant="outline" disabled={target.busy || target.id <= 0} onClick={openAdhoc}><SquareTerminal size={14} />手动请求</Button><Button size="sm" variant="destructive" disabled={!active} onClick={() => runAction("task.stop", target.name)}><CircleStop size={14} />停止当前任务</Button></div></CardHeader><CardContent><div className="status-grid"><div className="status-block"><div className="status-heading"><span>排队任务</span><Badge variant={stateVariant(target.queue.state)}>{statusLabel(target.queue.state)}</Badge></div><div className="metrics"><span>尝试 {target.queue.attempts} 次</span><span>{queueBusy ? "失败后立即继续" : "当前未排队"}</span></div><p className="muted small">最近尝试：{formatTime(target.queue.last_attempt)} · 完成：{formatTime(target.queue.finished_at)}</p>{target.queue.last_error && <p className="error-text small"><AlertTriangle size={13} />{target.queue.last_error}</p>}</div><div className="status-block"><div className="status-heading"><span>保活</span><Badge variant={stateVariant(target.keepalive.state)}>{statusLabel(target.keepalive.state)}</Badge></div><div className="metrics"><span>请求 {target.keepalive.requests} 次</span><span>下次 {countdown(target.keepalive.next_request)}</span></div><p className="muted small">最近成功：{formatTime(target.keepalive.last_success)} · 失败：{formatTime(target.keepalive.last_failure)}</p>{target.keepalive.last_error && <p className="error-text small"><AlertTriangle size={13} />{target.keepalive.last_error}</p>}</div></div></CardContent></Card>
}
