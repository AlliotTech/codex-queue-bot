...
import { AdhocRunDialog } from "./components/adhoc-run-dialog"
...
function SetupPage({ suggested, onComplete }: { suggested: string; onComplete: (auth: Auth) => void }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      onComplete({ username: result.username })
    } catch (err) {
...
function LoginPage({ onLoggedIn }: { onLoggedIn: (auth: Auth) => void }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      onLoggedIn({ username: result.username })
...
function connectionTone(state: Connection) {
...
function ConsoleSidebar({ view, setView }: { view: "dashboard" | "config"; setView: (view: "dashboard" | "config") => void }) {
  return <aside className="app-sidebar">
    <div className="sidebar-brand">
      <div className="sidebar-logo"><SquareTerminal size={19} /></div>
      <div><strong>Codex Queue</strong><span>Operations</span></div>
    </div>
    <div className="sidebar-label">工作区</div>
    <nav className="sidebar-nav" aria-label="主导航">
      <button type="button" className={view === "dashboard" ? "active" : ""} aria-current={view === "dashboard" ? "page" : undefined} onClick={() => setView("dashboard")}><Activity size={17} /><span>运行状态</span></button>
      <button type="button" className={view === "config" ? "active" : ""} aria-current={view === "config" ? "page" : undefined} onClick={() => setView("config")}><Settings size={17} /><span>配置</span></button>
    </nav>
    <div className="sidebar-spacer" />
    <div className="sidebar-footnote"><span className="status-dot" />实时状态已启用</div>
  </aside>
}

function ConsoleHeader({ username, view, onLogout }: { username: string; view: "dashboard" | "config"; onLogout: () => void }) {
  return <header className="topbar"><div className="topbar-heading"><div className="eyebrow"><span>CODEX OPERATIONS</span> <span className="eyebrow-separator">/</span> {view === "dashboard" ? "MONITOR" : "SETTINGS"}</div><h1>{view === "dashboard" ? "运行控制台" : "配置管理"}</h1><p>{view === "dashboard" ? "实时监控任务、连接与目标状态" : "管理运行参数、消息入口与管理员账号"}</p></div><div className="topbar-actions"><span className="session-pill"><span className="status-dot" />安全会话</span><span className="user-chip"><span className="avatar-mark"><ShieldCheck size={14} /></span>{username}</span><Button variant="ghost" size="sm" onClick={onLogout}><LogOut size={15} />退出</Button></div></header>
}

function ConsoleLayout({ username, view, setView, onLogout, children }: { username: string; view: "dashboard" | "config"; setView: (view: "dashboard" | "config") => void; onLogout: () => void; children: ReactNode }) {
  return <main className="app-shell"><ConsoleSidebar view={view} setView={setView} /><div className="app-main"><ConsoleHeader username={username} view={view} onLogout={onLogout} /><div className="app-content">{children}</div></div></main>
}

function TargetRow({ target, selected, toggle, runAction, openAdhoc }: { target: Target; selected: boolean; toggle: () => void; runAction: (actionName: string, target: string) => void; openAdhoc: () => void }) {
  const queueBusy = target.queue.state === "running"
  const keepaliveBusy = target.keepalive.state !== "stopped"
  const active = queueBusy || keepaliveBusy || target.adhoc_running
  return <article className={`target-row${selected ? " selected" : ""}`}>
    <div className="target-identity">
      <Checkbox aria-label={`选择 ${target.name}`} checked={selected} onChange={toggle} />
      <div className="target-copy">
        <div className="target-name-line"><h3>{target.name}</h3>{target.adhoc_running && <Badge>手动请求中</Badge>}</div>
        <p className="muted">{target.model} · {target.api_host}</p>
      </div>
    </div>
    <div className="target-status-cell">
      <div className="target-status-title"><span>排队任务</span><Badge variant={stateVariant(target.queue.state)}>{statusLabel(target.queue.state)}</Badge></div>
      <div className="target-status-meta"><span>尝试 {target.queue.attempts} 次</span><span>{queueBusy ? "失败后立即继续" : "当前未排队"}</span></div>
      <p className="target-status-detail">最近 {formatTime(target.queue.last_attempt)} · 完成 {formatTime(target.queue.finished_at)}</p>
      {target.queue.last_error && <p className="error-text small"><AlertTriangle size={13} />{target.queue.last_error}</p>}
    </div>
    <div className="target-status-cell">
      <div className="target-status-title"><span>保活</span><Badge variant={stateVariant(target.keepalive.state)}>{statusLabel(target.keepalive.state)}</Badge></div>
      <div className="target-status-meta"><span>请求 {target.keepalive.requests} 次</span><span>下次 {countdown(target.keepalive.next_request)}</span></div>
      <p className="target-status-detail">成功 {formatTime(target.keepalive.last_success)} · 失败 {formatTime(target.keepalive.last_failure)}</p>
      {target.keepalive.last_error && <p className="error-text small"><AlertTriangle size={13} />{target.keepalive.last_error}</p>}
    </div>
    <div className="target-row-actions">
      <Button size="icon" variant="default" aria-label="开始挤队列" title="开始挤队列" disabled={target.adhoc_running} onClick={() => runAction("queue.start", target.name)}><Play size={14} /></Button>
      <Button size="icon" variant="outline" aria-label="开始保活" title="开始保活" disabled={target.adhoc_running} onClick={() => runAction("keepalive.start", target.name)}><Radio size={14} /></Button>
      <Button size="icon" variant="outline" aria-label="手动请求" title="手动请求" disabled={target.busy || target.id <= 0} onClick={openAdhoc}><SquareTerminal size={14} /></Button>
      <Button size="icon" variant="destructive" aria-label="停止当前任务" title="停止当前任务" disabled={!active} onClick={() => runAction("task.stop", target.name)}><CircleStop size={14} /></Button>
    </div>
  </article>
}

function DashboardPage({ username, onLogout, setView }: { username: string; onLogout: () => void; setView: (view: "dashboard" | "config") => void }) {
  ...
  const [adhocTarget, setAdhocTarget] = useState<Target | null>(null)
  ...
  const runAction = async (actionName: string, targets: string[]) => {
    ...
    try {
      const result = await action(actionName, targets)
  ...
  return <ConsoleLayout username={username} view="dashboard" setView={setView} onLogout={onLogout}><RestartBanner fields={data.restart_fields || []} /><section className="page-heading dashboard-heading"><div><div className="section-kicker">OVERVIEW</div><h2>运行概览</h2><p className="muted">监控连接健康度与任务吞吐，状态会通过 SSE 自动更新。</p></div><div className="page-heading-actions"><span className={`live-pill ${connectionTone(connection)}`}><span className={`status-dot ${connectionTone(connection)}`} />{statusLabel(connection)}</span><Button variant="outline" size="sm" onClick={() => void load()}><RefreshCw size={14} />刷新状态</Button></div></section><section className="overview-grid"><Card><CardContent className="overview-card"><div className="overview-icon tone-blue"><Wifi size={19} /></div><div><span className="muted">SSE 连接</span><strong>{statusLabel(connection)}</strong></div></CardContent></Card><Card><CardContent className="overview-card"><div className="overview-icon tone-violet"><Radio size={19} /></div><div><span className="muted">OpenILink</span><strong>{statusLabel(data.openilink.state)}</strong>{data.openilink.error && <small className="error-text">{data.openilink.error}</small>}</div></CardContent></Card><Card><CardContent className="overview-card"><div className="overview-icon tone-sky"><Send size={19} /></div><div><span className="muted">Telegram</span><strong>{statusLabel(data.telegram.state)}</strong>{data.telegram.error && <small className="error-text">{data.telegram.error}</small>}</div></CardContent></Card><Card><CardContent className="overview-card"><div className="overview-icon tone-amber"><ArrowUp size={19} /></div><div><span className="muted">并发进程</span><strong>{data.concurrency.current} / {data.concurrency.max}</strong></div></CardContent></Card><Card><CardContent className="overview-card"><div className="overview-icon tone-emerald"><Activity size={19} /></div><div><span className="muted">服务 / 配置版本</span><strong>{data.version} · r{data.config_revision}</strong></div></CardContent></Card></section><section className="toolbar"><div className="toolbar-title"><div><div className="section-kicker">TARGETS</div><h2>目标</h2><p className="muted">每个目标同时只运行一个任务；手动请求要求目标当前空闲。</p></div><Badge variant="outline">{data.targets.length} 个目标</Badge></div>{data.targets.length > 0 && <div className="toolbar-actions"><Button variant="outline" size="sm" onClick={selectAll}>{selectedAll ? "取消全选" : "全选"}</Button><Button variant="outline" size="sm" disabled={!selected.length || busy} onClick={() => runAction("queue.start", selected)}><Play size={14} />开始挤队列</Button><Button variant="outline" size="sm" disabled={!selected.length || busy} onClick={() => runAction("keepalive.start", selected)}><Radio size={14} />开始保活</Button><Button variant="outline" size="sm" disabled={!selected.length || busy} onClick={() => runAction("task.stop", selected)}><CircleStop size={14} />停止当前任务</Button><AlertDialog open={confirmStop} onOpenChange={setConfirmStop} title="停止所有目标？" description="将停止全部目标当前正在运行的任务。此操作不会修改配置。" onConfirm={() => { void runAction("task.stop", []) }}><Button variant="destructive" size="sm" disabled={busy}><CircleStop size={14} />停止全部</Button></AlertDialog></div>}</section>{notice && <div className="notice" role="status">{notice}</div>}{visibleTargets.length === 0 ? <Card className="empty-state"><CardContent><Database size={34} /><h2>还没有 target</h2><p className="muted">先添加一个 API 目标，保存后会立即出现在这里，无需重启。</p><Button onClick={() => setView("config")}><Plus size={15} />前往配置 target</Button></CardContent></Card> : <section className="target-list" aria-label="目标列表"><div className="target-list-head" aria-hidden="true"><span>目标</span><span>排队任务</span><span>保活</span><span>操作</span></div>{visibleTargets.map(target => <TargetRow key={target.id || target.name} target={target} selected={selected.includes(target.name)} toggle={() => setSelected(items => items.includes(target.name) ? items.filter(item => item !== target.name) : [...items, target.name])} runAction={(name, targetName) => void runAction(name, [targetName])} openAdhoc={() => setAdhocTarget(target)} />)}</section>}<AdhocRunDialog target={adhocTarget} open={adhocTarget !== null} onOpenChange={open => { if (!open) setAdhocTarget(null) }} /><footer><span>最后更新：{formatTime(data.generated_at)}</span><span>运行状态只展示 API 主机名；完整 URL 仅在鉴权后的配置页可见。</span></footer></ConsoleLayout>
}
...
function ConfigNavigation({ section, setSection }: { section: ConfigSection; setSection: (section: ConfigSection) => void }) {
  const items: Array<[ConfigSection, string, typeof Database]> = [["targets", "Targets", Database], ["tasks", "任务与保活", SlidersHorizontal], ["openilink", "消息入口", Link2], ["runtime", "Web / 运行文件", Server], ["account", "管理员账号", UserCog]]
...
function ConfigurationPage({ username, onLogout, setView }: { username: string; onLogout: () => void; setView: (view: "dashboard" | "config") => void }) {
  ...
  const saveCodex = () => {
    ...
    void runSave(() => updateCodex(data.codex), "Codex 配置已保存")
  }
  ...
  const telegramField = <K extends keyof Configuration["telegram"]>(key: K, value: Configuration["telegram"][K]) => setData({ ...data, telegram: { ...data.telegram, [key]: value } })
  ...
  return <ConsoleLayout username={username} view="config" setView={setView} onLogout={onLogout}><RestartBanner fields={data.restart_fields} /><div className="config-layout"><ConfigNavigation section={section} setSection={value => { setSection(value); setEditing(null); setNotice(""); setError("") }} /><section className="config-content"><div className="config-content-header"><Button variant="ghost" size="sm" onClick={() => setView("dashboard")}><ArrowLeft size={15} />返回运行状态</Button><span className="muted">配置版本 r{data.revision}</span></div>{notice && <div className="notice" role="status">{notice}</div>}{error && <div className="error-banner" role="alert"><AlertTriangle size={16} />{error}</div>}
    ...
    {section === "openilink" && <div className="stack-lg">
      ...
      <Card><CardHeader><CardTitle>OpenILink</CardTitle></CardHeader><CardContent className="form-grid">
...
function AuthenticatedApp({ auth, onLogout }: { auth: Auth; onLogout: () => void }) {
  ...
  return view === "dashboard" ? <DashboardPage username={auth.username} onLogout={onLogout} setView={setView} /> : <ConfigurationPage username={auth.username} onLogout={onLogout} setView={setView} />
}
...
export default function App() {
  ...
  useEffect(() => {
    setupStatus().then(status => {
      ...
      return session().then(result => setAuth({ username: result.username })).catch(() => undefined)
    }).catch(() => undefined).finally(() => setBooting(false))
  }, [])
  const doLogout = useCallback(async () => { if (auth) await logout().catch(() => undefined); setAuth(null) }, [auth])
