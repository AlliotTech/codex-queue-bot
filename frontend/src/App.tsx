...
import {
  ...
  Save, Send, Server, Settings, ShieldCheck, SlidersHorizontal, Timer, Trash2,
...
function ConfigurationPage({ csrf, username, onLogout, setView }: { csrf: string; username: string; onLogout: () => void; setView: (view: "dashboard" | "config") => void }) {
  ...
  return <main className="app-shell"><ConsoleHeader username={username} view="config" setView={setView} onLogout={onLogout} /><RestartBanner fields={data.restart_fields} /><div className="config-layout"><ConfigNavigation section={section} setSection={value => { setSection(value); setEditing(null); setNotice(""); setError("") }} /><section className="config-content"><div className="config-content-header"><Button variant="ghost" size="sm" onClick={() => setView("dashboard")}><ArrowLeft size={15} />返回运行状态</Button><span className="muted">配置版本 r{data.revision}</span></div>{notice && <div className="notice" role="status">{notice}</div>}{error && <div className="error-banner" role="alert"><AlertTriangle size={16} />{error}</div>}
    ...
    {section === "tasks" && <div className="stack-lg"><div><h2>任务与保活</h2><p className="muted">保存后，新请求和最大并发立即采用新参数；等待中的重试和保活会从保存时刻重新随机倒计时。</p></div><Card><CardContent className="form-grid padded"><label>请求超时（秒）<Input type="number" min={1} value={data.codex.request_timeout_seconds} onChange={event => codexField("request_timeout_seconds", Number(event.target.value))} /></label><label>最大并发（立即生效）<Input type="number" min={1} value={data.codex.max_parallel} onChange={event => codexField("max_parallel", Number(event.target.value))} /></label><label>重试最小秒数<Input type="number" min={1} value={data.codex.retry_min_seconds} onChange={event => codexField("retry_min_seconds", Number(event.target.value))} /></label><label>重试最大秒数<Input type="number" min={1} value={data.codex.retry_max_seconds} onChange={event => codexField("retry_max_seconds", Number(event.target.value))} /></label><label>保活最小秒数<Input type="number" min={1} value={data.codex.keepalive_min_seconds} onChange={event => codexField("keepalive_min_seconds", Number(event.target.value))} /></label><label>保活最大秒数<Input type="number" min={1} value={data.codex.keepalive_max_seconds} onChange={event => codexField("keepalive_max_seconds", Number(event.target.value))} /></label><label>推理强度<select value={data.codex.reasoning_effort} onChange={event => codexField("reasoning_effort", event.target.value)}><option value="low">low</option><option value="medium">medium</option><option value="high">high</option><option value="xhigh">xhigh</option></select></label><label>成功消息<Input value={data.codex.success_message} onChange={event => codexField("success_message", event.target.value)} /></label><label className="span-2">全局 Codex overrides<textarea value={data.codex.config_overrides.join("\n")} onChange={event => codexField("config_overrides", splitLines(event.target.value))} placeholder="每行一个 key=value" /></label><div className="form-actions span-2"><Button onClick={saveCodex} disabled={busy}><Save size={15} />保存任务参数</Button></div></CardContent></Card></div>}
    {section === "openilink" && <div className="stack-lg">
      <div><h2>消息入口</h2><p className="muted">OpenILink 与 Telegram 可同时启用；连接配置保存后需要重启服务。</p></div>
      <Card><CardHeader><CardTitle>OpenILink</CardTitle></CardHeader><CardContent className="form-grid">
        ...
        <div className="form-actions span-2"><Button disabled={busy} onClick={() => void runSave(async () => { const result = await updateOpenILink(csrf, { enabled: data.openilink.enabled, base_url: data.openilink.base_url, token, clear_token: clearToken, allowed_user_ids: data.openilink.allowed_user_ids, http_timeout_seconds: data.openilink.http_timeout_seconds }); setToken(""); setClearToken(false); return result }, "OpenILink 配置已保存，重启后生效")}><Save size={15} />保存 OpenILink</Button></div>
      </CardContent></Card>
      <Card><CardHeader><CardTitle>Telegram Bot</CardTitle></CardHeader><CardContent className="form-grid">
        ...
        <div className="form-actions span-2"><Button disabled={busy} onClick={() => void runSave(async () => { const result = await updateTelegram(csrf, { enabled: data.telegram.enabled, base_url: data.telegram.base_url, token: telegramToken, clear_token: clearTelegramToken, allowed_user_ids: data.telegram.allowed_user_ids, http_timeout_seconds: data.telegram.http_timeout_seconds, poll_timeout_seconds: data.telegram.poll_timeout_seconds }); setTelegramToken(""); setClearTelegramToken(false); return result }, "Telegram 配置已保存，重启后生效")}><Save size={15} />保存 Telegram</Button></div>
      </CardContent></Card>
    </div>}
